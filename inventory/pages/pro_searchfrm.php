...
<?php
          ...
          <?php 
            $query = "
            SELECT PRODUCT_ID, PRODUCT_CODE, NAME,DESCRIPTION, COUNT(`QTY_STOCK`) AS 'QTY_STOCK',PRICE, c.CNAME FROM product p join category c on p.CATEGORY_ID=c.CATEGORY_ID WHERE PRODUCT_CODE ='{$_GET['id']}'
            ";
            $result = mysqli_query($db, $query) or die(mysqli_error($db));
