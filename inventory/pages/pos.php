...
<!-- END TAB PANE AREA - ANG UNOD KA TABS ARA SA TABPANE.PHP -->
        ...
        <br />  
        <?php 
        if(!isset($_SESSION['pointofsale']) || count($_SESSION['pointofsale'])==0){
          $hidden = 'hidden';
        }else{
          $hidden = '';
        }
        ?>
        <div class="card shadow mb-4 col-md-12" <?=$hidden?>>
...
<form role="form" method="post" action="pos_transac.php?action=add">
  ...
  <input type="hidden" name="role" value="<?php echo $_SESSION['JOB_TITLE']; ?>">
        ...
        <tr>  
          ...
          <!-- <td> -->
            <input type="hidden" name="prod_qty[]" value="<?php echo $product['prod_qty']; ?>">
